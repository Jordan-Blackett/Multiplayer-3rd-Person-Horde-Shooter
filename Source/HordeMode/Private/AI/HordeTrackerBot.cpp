...
FAutoConsoleVariableRef CVARDebugTrackerBotDrawing(
	TEXT("HORDE.DebugTrackerBot"),
	DebugTrackerBotDrawing,
	TEXT("Draw Debug Lines for TrackerBot"),
	ECVF_Cheat);

// Sets default values
AHordeTrackerBot::AHordeTrackerBot()
{
	...
	HealthComp->OnHealthChanged.AddDynamic(this, &AHordeTrackerBot::HandleTakeDamage);
...
void AHordeTrackerBot::BeginPlay()
{
	Super::BeginPlay();
	...
	if (Role == ROLE_Authority)
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle_CheckPowerLevel, this, &AHordeTrackerBot::OnCheckNearbyBots, 1.0f, true);
	}
	
}

void AHordeTrackerBot::HandleTakeDamage(UHordeHealthComponent* OwningHealthComp, float Health, float HealthDelta, const class UDamageType* DamageType, class AController* InstigatedBy, AActor* DamageCauser)
{
	if (MatInst == nullptr)
	{
...
FVector AHordeTrackerBot::GetNextPathPoint()
{
	...
	for (FConstPawnIterator It = GetWorld()->GetPawnIterator(); It; ++It)
	{
		...
		if (TestPawn == nullptr || UHordeHealthComponent::IsFriendly(TestPawn, this))
		{
			continue;
		}

		UHordeHealthComponent* TestPawnHealthComp = Cast<UHordeHealthComponent>(TestPawn->GetComponentByClass(UHordeHealthComponent::StaticClass()));
	...
	if (BestTarget)
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle_RefreshPath, this, &AHordeTrackerBot::RefreshPath, 5.0f, false);
...
void AHordeTrackerBot::SelfDestruct()
{
	if (bExploded)
	{
		return;
...
// Called every frame
void AHordeTrackerBot::Tick(float DeltaTime)
{
...
void AHordeTrackerBot::NotifyActorBeginOverlap(AActor* OtherActor)
{
	Super::NotifyActorBeginOverlap(OtherActor);

	if (!bStartedSelfDestruction && !bExploded)
	{
		AHordeCharacter* PlayerPawn = Cast<AHordeCharacter>(OtherActor);
		if (PlayerPawn && !UHordeHealthComponent::IsFriendly(OtherActor, this))
		{
			...
			if (Role == ROLE_Authority)
			{
				...
				GetWorldTimerManager().SetTimer(TimerHandle_SelfDamage, this, &AHordeTrackerBot::DamageSelf, SelfDamageInterval, true, 0.0f);
			}
...
void AHordeTrackerBot::OnCheckNearbyBots()
{
	...
	for (FOverlapResult Result : Overlaps)
	{
		...
		AHordeTrackerBot* Bot = Cast<AHordeTrackerBot>(Result.GetActor());
...
void AHordeTrackerBot::RefreshPath()
{
	NextPathPoint = GetNextPathPoint();
}
